...
**Hashes count content, not what a machine left behind.** `patched` compares `skills/<name>` against `upstream/<name>` by tree hash, so anything present on only one device would flip the flag back and forth as machines take turns syncing. The hash therefore asks git what belongs to the skill — tracked files plus untracked ones git does not ignore — which honors a skill's own nested `.gitignore`; video-talkcraft ships one declaring the files its tooling regenerates per machine, and no fixed list of names could have known them. Outside a work tree (a local source directory) it walks instead, skipping `node_modules`, `__pycache__`, virtualenvs, tool caches, `.pyc` and `.DS_Store`. Symlinks resolving outside the tree are skipped either way.
